...
import { Download, Edit2, Plus, Search, Trash2, Shield } from "lucide-react";
...
export const Route = createFileRoute("/admin/ex-service-manager")({
  component: ExServiceManagerPage,
});
...
const QK = ["admin", "ex-services"] as const;
const MODULE = "Ex-Service Manager";
const ENTITY = "ex_services";

function rowToItem(r: Record<string, unknown>): ExService {
  return {
    id: String(r.id),
    name: String(r.name ?? ""),
...
function useExServices() {
  ...
  const { data: items = [] } = useQuery({
    queryKey: QK,
    queryFn: async (): Promise<ExService[]> => {
  ...
  type Payload = Omit<ExService, "id">;
  ...
  const addMut = useMutation({
    mutationFn: async (p: Payload) => {
      ...
      const { error } = await supabase.from("ex_services" as never).insert(toRow(p) as never);
      if (error) throw error;
  ...
  const deleteMut = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("ex_services" as never).delete().eq("id", id);
      if (error) throw error;
...
function ExServiceManagerPage() {
  const { items, addMut, updateMut, toggleMut, deleteMut } = useExServices();
  const [query, setQuery] = useState("");
  const [addOpen, setAddOpen] = useState(false);
  const [editing, setEditing] = useState<ExService | null>(null);
  const [deleting, setDeleting] = useState<ExService | null>(null);
  ...
  return (
    <div>
      ...
      <ExServiceFormDialog
        ...
        title="Add New Ex-Service"
        onSubmit={async (p) => {
          try {
            await addMut.mutateAsync(p);
            toast.success("Ex-service added");
            return null;
          } catch (e) {
            return e instanceof Error ? e.message : "Could not add ex-service";
          }
        }}
      />
      ...
      <ExServiceFormDialog
        ...
        title="Edit Ex-Service"
        onSubmit={async (p) => {
          if (!editing) return null;
          try {
            await updateMut.mutateAsync({ id: editing.id, p });
            toast.success("Ex-service updated");
            setEditing(null);
            return null;
          } catch (e) {
            return e instanceof Error ? e.message : "Could not update ex-service";
          }
        }}
      />
            ...
            <AlertDialogAction
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  toast.success("Ex-service deleted");
                  setDeleting(null);
                } catch (e) {
...
function ExServiceFormDialog({
  ...
  title: string;
  initial?: ExService | null;
  onSubmit: (p: Omit<ExService, "id">) => Promise<string | null>;
}) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Rank or category of ex-servicemen used across employee records.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          ...
          <div className="grid gap-2">
            ...
            <Textarea
              ...
              placeholder="e.g. Indian Army - JCO / NCO"
              rows={3}
            />
          </div>
