...
import { Download, Edit2, Plus, Search, Trash2, Languages as LangIcon } from "lucide-react";
...
const QK = ["admin", "languages"] as const;
const MODULE = "Language Manager";
const ENTITY = "languages";

function rowToItem(r: Record<string, unknown>): Language {
  return {
    id: String(r.id),
    name: String(r.name ?? ""),
...
function useLanguages() {
  ...
  type Payload = Omit<Language, "id">;
  const toRow = (p: Payload) => ({ name: p.name.trim(), enabled: p.enabled });

  const addMut = useMutation({
    mutationFn: async (p: Payload) => {
      if (!p.name.trim()) throw new Error("Name is required");
      const { error } = await supabase.from("languages" as never).insert(toRow(p) as never);
      if (error) throw error;
  ...
  const updateMut = useMutation({
    mutationFn: async ({ id, p }: { id: string; p: Payload }) => {
      const { error } = await supabase.from("languages" as never).update(toRow(p) as never).eq("id", id);
      if (error) throw error;
  ...
  const toggleMut = useMutation({
    mutationFn: async ({ id, enabled }: { id: string; enabled: boolean }) => {
      const { error } = await supabase.from("languages" as never).update({ enabled } as never).eq("id", id);
      if (error) throw error;
  ...
  const deleteMut = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("languages" as never).delete().eq("id", id);
      if (error) throw error;
...
function LanguageManagerPage() {
  const { items, addMut, updateMut, toggleMut, deleteMut } = useLanguages();
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            onClick={() =>
              downloadCsv(
                "languages",
                filtered.map((i) => ({ name: i.name, enabled: i.enabled ? "Yes" : "No" })),
                [{ key: "name", header: "Name" }, { key: "enabled", header: "Enabled" }],
              )
            }
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={3} className="px-5 py-12 text-center text-sm text-muted-foreground">No languages found.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <LanguageFormDialog
        ...
        onSubmit={async (p) => {
          try {
            ...
            return e instanceof Error ? e.message : "Could not add language";
          }
        }}
      />
      ...
      <LanguageFormDialog
        ...
        onSubmit={async (p) => {
          ...
          try {
            ...
            return e instanceof Error ? e.message : "Could not update language";
          }
        }}
      />
...
function LanguageFormDialog({
  ...
  onSubmit: (p: Omit<Language, "id">) => Promise<string | null>;
}) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Used as a dropdown across employee records.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          <div className="grid gap-2">
            <Label>Name</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Hindi" />
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              setSaving(true);
              const err = await onSubmit({ name, enabled });
              setSaving(false);
