...
const CALC_VALUES: CalcType[] = ["lumpsum", "per_duty_amount", "total_amount"];
const STATUS_VALUES: Status[] = ["active", "paused", "completed", "cancelled"];
...
function downloadTemplate(types: AType[]) {
  const headers = [
    "employee_code", "addition_type", "addition_date",
    "calculation_type", "amount", "installments", "description", "status",
  ];
  const sample = [
    "EMP-001",
    types[0]?.code ?? types[0]?.name ?? "BONUS",
  ...
  // Reference sheet with valid types
  const refRows: (string | number)[][] = [["Valid Addition Types (use code or name)"], ["code", "name"]];
  types.forEach((t) => refRows.push([t.code, t.name]));
  refRows.push([""], ["Valid calculation_type"], ["lumpsum"], ["per_duty_amount"], ["total_amount"]);
  refRows.push([""], ["Valid status"], ["active"], ["paused"], ["completed"], ["cancelled"]);
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(refRows), "Reference");

  XLSX.writeFile(wb, "additions-bulk-template.xlsx");
}

function BulkUploadDialog({
  open, onOpenChange, types, emps, onDone,
}: {
  open: boolean;
  ...
  const empByCode = useMemo(() => {
    ...
    emps.forEach((e) => { if (e.employee_code) m.set(e.employee_code.toLowerCase(), e); });
    return m;
  }, [emps]);
  const typeByKey = useMemo(() => {
    const m = new Map<string, AType>();
    types.forEach((t) => {
      if (t.code) m.set(t.code.toLowerCase(), t);
  ...
  async function handleFile(f: File) {
    ...
    try {
      ...
      const parsed: BulkRow[] = json.map((r, i) => {
        const get = (k: string) => {
          const key = Object.keys(r).find((kk) => kk.trim().toLowerCase() === k.toLowerCase());
          return key ? r[key] : "";
        };
        const employee_code = String(get("employee_code") ?? "").trim();
        const addition_type = String(get("addition_type") ?? "").trim();
        const addition_date = excelDateToISO(get("addition_date"));
        const calculation_type = String(get("calculation_type") ?? "lumpsum").trim().toLowerCase();
        const amount = Number(get("amount") ?? 0);
        const installments = Math.max(1, parseInt(String(get("installments") ?? "1"), 10) || 1);
        const description = String(get("description") ?? "").trim();
        const status = String(get("status") ?? "active").trim().toLowerCase();

        const emp = empByCode.get(employee_code.toLowerCase());
        const type = typeByKey.get(addition_type.toLowerCase());

        const errors: string[] = [];
        if (!employee_code) errors.push("employee_code missing");
        else if (!emp) errors.push(`unknown employee_code "${employee_code}"`);
        if (!addition_type) errors.push("addition_type missing");
        else if (!type) errors.push(`unknown addition_type "${addition_type}"`);
        if (!addition_date) errors.push("invalid addition_date");
        if (!CALC_VALUES.includes(calculation_type as CalcType)) errors.push(`bad calculation_type "${calculation_type}"`);
        if (!Number.isFinite(amount) || amount < 0) errors.push("invalid amount");
        if (!STATUS_VALUES.includes(status as Status)) errors.push(`bad status "${status}"`);

        return {
          rowNum: i + 2,
          employee_code, addition_type, addition_date,
          calculation_type, amount, installments, description, status,
          candidate_id: emp?.id, addition_type_id: type?.id,
  ...
  async function handleUpload() {
    ...
    try {
      const payload = validRows.map((r) => ({
        ...
        addition_name: `${r.employee_code} - ${r.addition_type} - ${r.addition_date}`,
        calculation_type: r.calculation_type as CalcType,
        amount: r.amount,
      ...
      void logActivity({
        ...
        entityLabel: `Bulk upload: ${payload.length} additions`,
      });
      toast.success(`Uploaded ${payload.length} addition${payload.length === 1 ? "" : "s"}`);
      onDone();
      reset();
      onOpenChange(false);
    } catch (e) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) reset(); onOpenChange(o); }}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>Bulk Upload Additions</DialogTitle>
          <DialogDescription>
            Download the template, fill it in, then upload. Required columns:
            employee_code, addition_type, addition_date, calculation_type, amount, installments.
          </DialogDescription>
        </DialogHeader>

        <div className="flex flex-wrap items-center gap-2">
          <Button type="button" variant="outline" onClick={() => downloadTemplate(types)}>
        ...
        {rows.length > 0 && (
          <div className="max-h-80 overflow-auto rounded-lg border border-border">
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-secondary/80 text-left text-[10px] uppercase tracking-[0.1em] text-muted-foreground">
                <tr>
                  <th className="px-2 py-1.5">Row</th>
                  <th className="px-2 py-1.5">Emp</th>
                  <th className="px-2 py-1.5">Type</th>
                  <th className="px-2 py-1.5">Date</th>
                  <th className="px-2 py-1.5">Calc</th>
              ...
              <tbody className="divide-y divide-border">
                {rows.map((r) => (
                  <tr key={r.rowNum} className={r.error ? "bg-destructive/5" : ""}>
                    <td className="px-2 py-1 font-mono">{r.rowNum}</td>
                    <td className="px-2 py-1">{r.employee_code}</td>
                    <td className="px-2 py-1">{r.addition_type}</td>
                    <td className="px-2 py-1">{r.addition_date || "—"}</td>
                    <td className="px-2 py-1">{r.calculation_type}</td>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleUpload} disabled={uploading || parsing || validRows.length === 0}>
