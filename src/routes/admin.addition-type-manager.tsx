...
export const Route = createFileRoute("/admin/addition-type-manager")({
  component: AdditionTypeManagerPage,
});

type AType = { id: string; name: string; code: string; is_active: boolean; sort_order: number };

const QK = ["admin", "addition-types"] as const;

function slugify(s: string) {
  return s.toLowerCase().trim().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
}

function AdditionTypeManagerPage() {
  ...
  const addMut = useMutation({
    mutationFn: async (p: { name: string; code: string; is_active: boolean }) => {
      const { error } = await supabase.from("addition_types" as never).insert(p as never);
      if (error) throw error;
      void logActivity({ module: "Addition Type Manager", action: "create", entityType: "addition_types", entityLabel: p.name });
    },
    onSuccess: inv,
  });
  const updateMut = useMutation({
    mutationFn: async ({ id, p }: { id: string; p: Partial<AType> }) => {
      const { error } = await supabase.from("addition_types" as never).update(p as never).eq("id", id);
      if (error) throw error;
      void logActivity({ module: "Addition Type Manager", action: "update", entityType: "addition_types", entityId: id });
    },
    onSuccess: inv,
  });
  const deleteMut = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("addition_types" as never).delete().eq("id", id);
      if (error) throw error;
      void logActivity({ module: "Addition Type Manager", action: "delete", entityType: "addition_types", entityId: id });
    },
    onSuccess: inv,
  });
  ...
  const [editing, setEditing] = useState<AType | null>(null);
  const [deleting, setDeleting] = useState<AType | null>(null);
    ...
    <div>
      ...
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <Button onClick={() => setAddOpen(true)} className="h-10 rounded-lg bg-primary font-semibold text-primary-foreground hover:bg-primary/90">
          <Plus className="mr-1.5 h-4 w-4" /> Add Addition Type
        </Button>
      </div>
      ...
      <FormDialog
        open={addOpen} onOpenChange={setAddOpen} title="Add Addition Type"
        onSubmit={async (p) => {
      ...
      />
      <FormDialog
        open={!!editing} initial={editing} onOpenChange={(o) => !o && setEditing(null)} title="Edit Addition Type"
        onSubmit={async (p) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Used as the Addition Type dropdown on each employee addition.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          <div className="grid gap-2">
            <Label>Name</Label>
            <Input value={name} onChange={(e) => { setName(e.target.value); if (!initial) setCode(slugify(e.target.value)); }} placeholder="e.g. Performance Bonus" />
          </div>
          <div className="grid gap-2">
            <Label>Code</Label>
            <Input value={code} onChange={(e) => setCode(e.target.value)} placeholder="performance_bonus" />
          </div>
