          ...
          {isLoading ? (
            ...
            filtered.map((unit) => {
              ...
              return (
                ...
                >
                  {/* Header row: icon + unit name + action */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 min-w-0">
                      <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-emerald-100/80 text-emerald-700">
                        <Wallet className="h-4 w-4" />
                      </div>
                      <div className="min-w-0">
                        <div className="text-sm font-semibold text-foreground truncate">{unit.name || unit.code}</div>
                        <span className="mt-1 inline-flex rounded-md bg-secondary px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-wide text-foreground">
                          {unit.code || "—"}
                        </span>
                      </div>
                    ...
                    <div className="shrink-0">
                      {targetPeriod ? (
                        <Link
                          ...
                          params={{ unitId: unit.id }}
                          search={{ start: targetPeriod.period_start, end: targetPeriod.period_end }}
                          className="inline-flex items-center gap-1 rounded-full border border-border/60 bg-background px-3 py-1.5 text-xs font-semibold text-foreground transition hover:border-accent/50 hover:text-accent"
                          title="Show invoice"
                        >
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-medium text-foreground truncate">{unit.customer_name}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-semibold text-foreground">{unit.active_employee_count}</span>
                  </div>
...
function DashStat({
  ...
  return (
    <Cmp
      onClick={onClick}
      className={`group relative w-full text-left overflow-hidden rounded-2xl p-3 shadow-sm transition text-white hover:-translate-y-0.5 hover:shadow-md ${accentMap[accent]} ${active ? "ring-2 ring-offset-2 ring-white/60" : ""} ${onClick ? "cursor-pointer" : ""}`}
    >
      <div className="flex items-center gap-2">
        <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/25 text-white">
          <Icon className="h-3.5 w-3.5" />
        </div>
        <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/90">{label}</div>
      </div>
      <div className={`mt-1 font-display font-bold tabular-nums tracking-tight text-white ${compact ? "text-2xl" : "text-2xl"}`}>
        {loading ? <span className="text-white/60">—</span> : value.toLocaleString()}
      </div>
    </Cmp>
  );
}
