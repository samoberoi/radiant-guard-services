...
1. **Cost Component Manager**
   ...
   - Keep the base as: `Earned Gross - Earned Washing Allowance - Earned Conveyance Allowance`.
   - Make the label/description clearly attendance-based, not a fixed monthly amount.
...
3. **Payroll Compute Wages**
   ...
   - Calculate employee ESI at payroll time only as:
     `ceil(0.75% × (Earned Gross Salary - Earned Washing Allowance - Earned Conveyance Allowance))`
...
4. **Existing Kids Clinic contract data**
   ...
   - For Kids Clinic example, with full May attendance currently showing `T Days = 31.88`, gross `₹19,339`, actual-days base `31`:
     ...
     - Washing = `₹0`, Conveyance = `₹0`
     - ESI base = `₹19,888.49`
     - Employee ESI = `ceil(19,888.49 × 0.75%) = ceil(149.16) = ₹150`
     - Employer ESI = `ceil(19,888.49 × 3.25%) = ceil(646.38) = ₹647`
